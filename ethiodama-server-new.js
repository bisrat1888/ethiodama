...
(function checkTelebirrKey() {
  ...
  const looksValid = key.startsWith('-----BEGIN') && key.includes('\n') && key.trim().endsWith('-----END PRIVATE KEY-----');
  if (!looksValid) {
    console.warn('[telebirr] ⚠️  WARNING: privateKey does not look like a valid multi-line PEM. ' +
      'If set via env var, check that literal "\\n" got converted to real newlines.');
  } else {
    console.log('[telebirr] ✅ privateKey format looks valid (PEM, multi-line).');
  }
})();
...
function normalizePemKey(key) {
  if (!key) return key;
  // Render/env-var UIs often store pasted PEM keys with literal backslash-n
  // instead of real line breaks, which makes OpenSSL's key decoder fail
  // with "error:1E08010C:DECODER routines::unsupported". Fix that here.
  return key.includes('\\n') ? key.replace(/\\n/g, '\n') : key;
}
...
function encryptWithPublicKey(data) {
  ...
  try {
    ...
    const encrypted = publicEncrypt(
      { key: normalizePemKey(TELEBIRR_CONFIG.publicKey) || '', padding: constants.RSA_PKCS1_PADDING },
      Buffer.from(data)
    );
...
function signWithPrivateKey(data) {
  try {
    ...
    sign.update(data);
    return sign.sign(normalizePemKey(TELEBIRR_CONFIG.privateKey), 'base64');
  } catch (err) {
