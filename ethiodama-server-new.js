  ...
  try {
    const response = await axios.post(
      `${CHAPA_BASE_URL}/transaction/initialize`,
      {
        ...
        currency: 'ETB',
        email: playerEmail || 'player@ethiodama.com',
        first_name: playerName || 'Player',
        last_name: 'EthioDama',
        tx_ref: txRef,
...
io.on('connection', (socket) => {
  ...
  socket.on('initiate_payment', async ({ wager, name, email }) => {
    if (!WAGER_OPTIONS.includes(wager)) {
      return socket.emit('error_message', { message: 'የተሳሳተ የብር መጠን!' });
    }
    const result = await initiatePayment(socket.id, wager, name, email);
    if (result.success) {
      socket.emit('payment_url', { checkoutUrl: result.checkoutUrl, txRef: result.txRef, wager });
    } else {
      socket.emit('error_message', { message: 'ከChapa ጋር መገናኘት አልተቻለም።' });
    }
  });
  ...
  socket.on('join_queue_free', ({ wager }) => {
    if (!WAGER_OPTIONS.includes(wager)) return;
