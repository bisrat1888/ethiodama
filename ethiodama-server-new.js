...
const usedTelebirrReceipts = new Set(); // prevents the same receipt being reused (resets on server restart — fine for now, consider a DB for production)
...
async function verifyTelebirrReceipt(receiptNo, expectedAmount) {
  ...
  const fields = parseTelebirrReceiptHTML(resp.data);

  if (!fields.receiptNo && !fields.credited_party_acc_no) {
    return { valid: false, reason: 'Receipt አልተገኘም - ቁጥሩን አረጋግጥ' };
  }
...
io.on('connection', (socket) => {
  ...
  socket.on('verify_manual_receipt', async ({ receiptNo, wager }) => {
    try {
      ...
      if (result.valid) {
        socket.emit('payment_confirmed', { wager });
        joinQueue(socket, wager);
      } else {
        socket.emit('payment_failed', { message: result.reason });
      }
    } catch (err) {
